...
def main():
    ...
    stock = st.sidebar.text_input("Enter your stock code")
    start_date = st.sidebar.date_input('Start date')
    end_date = st.sidebar.date_input('End date')
    st.title("Stock Monitor")
    
    news_source = NewsDAO().getNews(stock,start_date,end_date)
    news_list = NewsDAO().create_news(news_source)
    news_df = pd.DataFrame(NewsDAO().getJsonList(news_list))

    price = getStock(stock,start_date,end_date)
    st.dataframe(price)
    st.line_chart(price)
    ac1 = price
    size = len(ac1)-1
